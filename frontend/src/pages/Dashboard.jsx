...
import { ArrowUpRight, ArrowDownRight, CreditCard, Plus, ArrowLeftRight, Landmark, Calendar } from 'lucide-react';
...
function Dashboard({ currency, raw, transactions, refreshData }) {
  ...
  const nextCard = () => {
    if (userCards) {
      setCardAnim('');
      setTimeout(() => { setCardIndex((p) => (p + 1) % userCards.length); setCardAnim('widget-anim-enter'); }, 10);
    }
  };
  const handleCardSelect = (idx) => {
    setCardAnim('');
    setTimeout(() => { setCardIndex(idx); setCardAnim('widget-anim-enter'); }, 10);
  };
  ...
  const nextCuenta = () => {
    if (userCuentas) {
      setCuentaAnim('');
      setTimeout(() => { setCuentaIndex((p) => (p + 1) % userCuentas.length); setCuentaAnim('widget-anim-enter'); }, 10);
    }
  };
  const handleCuentaSelect = (idx) => {
    setCuentaAnim('');
    setTimeout(() => { setCuentaIndex(idx); setCuentaAnim('widget-anim-enter'); }, 10);
  };
  ...
  const chartData = useMemo(() => {
    ...
    const getWeek = (date) => {
      ...
      return `Sem ${Math.ceil((((d - yearStart) / 86400000) + 1) / 7)}`;
    };
    periodTx.forEach(tx => {
      const d = new Date(tx.fecha);
      let key = timeFilter === 'Mensual'
  ...
  const pieData = useMemo(() => {
    const catMap = {};
    periodTx.filter(t => t.isExpense).forEach(g => {
  ...
  return (
    <>
      ...
      <div className="period-filter-bar">
        ...
        <span style={{ fontSize: '13px', color: 'var(--text-muted)', fontWeight: '500' }}>Período:</span>
      ...
      <div className="widgets-grid">
        ...
        <div>
          ...
          <button className="btn-add-card" onClick={() => setIsCardModalOpen(true)}>
            <CreditCard size={18} /> Agregar Tarjeta
          </button>
        </div>
      </div>
